...
const validatExpirienceInput = require('../../validation/experience');
const validatEducationInput = require('../../validation/education');

// @route GET /api/profile/test
// @desc Tests profile route
// @access Public
router.get('/test', (req, res) => res.status(200).json({msg: "Profile work"}));
...
router.get('/handle/:handle', (req, res) => {
    ...
    Profile.findOne({handle:req.params.handle})
            ...
            .then(profile => {
                if(!profile){
                    errors.noprofile = "There is no profile or this user";
                    res.status(404).json(errors);
                }
                    ...
                    .then(profile => {
                        ...
                        else
                        {
                            ...
                            Profile.findOne({ handle: profileFields.handle})
                                .then(profile => {
                                    if(profile){
                                        errors.handle = 'That andle already exists';
                                        res.status(400).json(errors);
                                    }
                                    //save profile
...
router.post('/experience', passport.authenticate('jwt', {session:false}), (req, res) => {
    const {errors, isValid} = validatExpirienceInput(req.body);
     //Check Validation
     if(!isValid){
        res.status(400).json(errors);
    }
